...
public final class LunaChatClient implements ClientModInitializer {
    ...
    private final Map<GuiMessage, GuiMessage> translations = new IdentityHashMap<>();
    private final Set<GuiMessage> arrivals = Collections.newSetFromMap(new IdentityHashMap<>());
    private final Set<GuiMessage> queued = Collections.newSetFromMap(new IdentityHashMap<>());
    ...
    private final Set<GuiMessage> localMessages = Collections.newSetFromMap(new IdentityHashMap<>());
    private boolean writingLocal;
    private KeyMapping translateKey, toggleKey, originalKey;
    ...
    @Override public void onInitializeClient() {
        ...
        KeyMapping.Category category = KeyMapping.Category.register(Identifier.fromNamespaceAndPath("luna_chat", "keys"));
        translateKey = KeyMappingHelper.registerKeyMapping(new KeyMapping("key.luna_chat.translate", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_F6, category));
        toggleKey = KeyMappingHelper.registerKeyMapping(new KeyMapping("key.luna_chat.toggle", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_F7, category));
        originalKey = KeyMappingHelper.registerKeyMapping(new KeyMapping("key.luna_chat.original", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_F8, category));
    ...
    public static GuiMessage displayLine(GuiMessage original) {
        return INSTANCE == null ? original : INSTANCE.translations.getOrDefault(original, original);
    }
    public static void received(GuiMessage line) {
    ...
    public static boolean handleChatKey(KeyEvent key) {
        LunaChatClient self = INSTANCE;
        if (self == null) return false;
        if (self.translateKey.matches(key)) { self.toggleClick(); return true; }
        if (self.toggleKey.matches(key)) { self.toggle(); return true; }
        if (self.originalKey.matches(key)) { self.restore(); return true; }
        return false;
    }

    private ChatAccess hud() { return (ChatAccess) Minecraft.getInstance().gui.hud.getChat(); }

    private void tick(Minecraft client) {
        while (translateKey.consumeClick()) if (client.gui.screen() == null) openPicker();
        while (toggleKey.consumeClick()) if (client.gui.screen() == null) toggle();
        while (originalKey.consumeClick()) if (client.gui.screen() == null) restore();
        if (client.level == null) return;
        Set<GuiMessage> alive = Collections.newSetFromMap(new IdentityHashMap<>());
    ...
    private void dispatch() {
        ...
        List<GuiMessage> alive = hud().luna$messages();
        ...
        worker.submit(() -> {
            try {
                ...
                Minecraft.getInstance().execute(() -> {
                    if (generation == epoch) {
                        List<GuiMessage> current = hud().luna$messages();
                        for (int i = 0; i < batch.size(); i++) {
                            ChatAccess.Selection s = batch.get(i).selection;
                            if (current.stream().noneMatch(line -> line == s.line())) continue;
                            Component replaced = ChatText.replace(s.line().content(), s.start(), s.end(), result.get(i));
                            GuiMessageTag indicator = new GuiMessageTag(0x7DB8FF, null,
                                    Component.literal("Luna 翻訳（自分の画面のみ）\n原文: ").append(s.line().content()), "Luna");
                            translations.put(s.line(), new GuiMessage(s.line().addedTime(), replaced, s.line().signature(), s.line().source(), indicator));
    ...
    private void login() {
        ...
        accountTask(() -> {
            ...
            Minecraft.getInstance().execute(() -> Util.getPlatform().openUri(uri));
        });
    }
    ...
    private static void notice(String text) {
        ...
        client.execute(() -> { if (client.player != null) client.gui.hud.setOverlayMessage(Component.literal("[Luna] " + text), true); });
    }
    ...
    public static boolean eligible(GuiMessage line) {
    ...
    public void translateDraft(String draft, java.util.function.Consumer<String> success, Runnable failure) {
        ...
        worker.submit(() -> {
            try {
                ...
                Minecraft.getInstance().execute(() -> { failure.run(); notice("入力文の英訳に失敗しました。原文は保持されています"); });
            }
        });
    }
    ...
    public void openPicker() {
        ...
        Minecraft client = Minecraft.getInstance();
        if (client.level != null) client.execute(() -> client.gui.setScreen(new net.minecraft.client.gui.screens.ChatScreen("", false)));
    }
    public void openSettings() {
        Minecraft client = Minecraft.getInstance();
        client.execute(() -> client.gui.setScreen(new LunaSettingsScreen(null)));
    }
    public void showUsage() {
        writingLocal = true;
        try { for (String line : usage.lines()) Minecraft.getInstance().gui.hud.getChat().addClientSystemMessage(Component.literal("[Luna] " + line)); }
